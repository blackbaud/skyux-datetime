...
export class SkyDateFormatter {
  ...
  public getDateFromString(dateString: string, format: string, strict: boolean): Date {
  ...
  public dateFromUserInput(inputValue: any, format: string, strict: boolean): Date {
    const _maxTwoDigitYear = 9999;
    const sep = '/';
    ...
    if (inputValue) {
      ...
      if (!date || valueParts.length !== 3) {
        let today = new Date(),
          month = today.getUTCMonth() + 1, //months are 0 based, but we need to process as 1 based
          day = today.getUTCDate(),
          thisYear = today.getUTCFullYear(),
          borderYear = _maxTwoDigitYear,
          year = thisYear,
          tmp = 0,

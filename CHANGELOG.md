...
- Added the public internal properties `buttonIsFocused`, `calendarIsFocused`, and `calendarIsVisible` to `SkyDatepickerComponent`, which indicate the focus and visibility states of datepicker elements. Added the public internal property `inputIsFocused` to `SkyDatepickerInputDirective`, which indicates the focus state of the datepicker input. [#77](https://github.com/blackbaud/skyux-datetime/pull/77)
